...
export default class DustStatus extends BaseCommand {
  ...
  async run(): Promise<void> {
    ...
    // capacity for a cNIGHT holder stay unavailable until moth can supply a
    // Cardano stake address, which it does not derive.
    ...
    this.outputSuccess({
      ...
      ...(result.truncated
        ? { truncated: true, note: 'stopped on the time budget — the list may be incomplete' }
        : {}),
