...
export function registerSpendTools(server: McpServer, deps: McpToolDeps, cfg: SpendToolConfig): void {
  ...
  if (cfg.allowBalancing) {
    server.registerTool(
      'balance_transaction',
      {
        ...
        inputSchema: {
          txHex: z.string().regex(/^([0-9a-fA-F]{2})+$/)
            .describe('Hex-encoded transaction to balance, as produced by the dApp/site endpoint.'),
  ...
  if (cfg.allowBalancing) {
    server.registerTool(
      ...
      {
        description:
          'Submit a pre-built, fully-balanced FinalizedTransaction (hex) to the network and return its transaction id. ' +
        ...
        inputSchema: {
          txHex: z.string().regex(/^([0-9a-fA-F]{2})+$/)
            .describe('Hex-encoded FinalizedTransaction (output of tx.serialize()), e.g. the finalizedHex returned by balance_transaction with submit=false.'),
        },
        annotations: {destructiveHint: true},
      },
      async ({txHex}): Promise<CallToolResult> => {
        try {
