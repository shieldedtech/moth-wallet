...
const DRAIN_ADDRESS = 'mn_addr_preprod1he0ty4u5vnmqdmvg85vgx9shxqh5snnvudszkr38ykqpkefu6xwsq30mjf';
...
function report(label: string, bytes: Uint8Array) {
  const s = summarizeConnectorTransaction(bytes, false, 'preprod');
...
describe('drain-shape probe: an unfunded output must always show as a spend', () => {
  ...
  it('surfaces the drain amount, token AND destination', async () => {
    // CRIT-001 reported an approval showing "no recipient, no amount and no
    // token". This pins all three for exactly the reported drain shape
    // (3000.000000 NIGHT to a third party); tx-summary-view formats the amount
    // and Approval.tsx renders the address.
  ...
  it('names the destination even when the wallet pays nothing extra', async () => {
    // The dangerous middle case: a dApp transaction the wallet need not top up
    // still moves tokens to someone. Amount-only screens call this "takes
    // nothing from your wallet"; the destination is the whole story.
    const intent = Intent.new(new Date(Date.now() + 60_000));
    intent.fallibleUnshieldedOffer = UnshieldedOffer.new([], [out(DRAIN)], []);
