...
```bash
# ask the chain: the seed's first UNSHIELDED transaction becomes the birthday
moth wallet import --name w --seed-hex --birthday-discover
...
There is no address for the indexer to index and no query that can answer "did
this seed receive shielded funds before block N" — so `--birthday-discover` returns
a height that is correct for unshielded and *unverified* for shielded. If the seed
received shielded funds before that block:

- the sync starts above them and never looks lower,
- they are simply absent from the balance — no error, no warning, nothing missing-looking,
- and DUST generated by NIGHT you cannot see is missing too.

Nothing is destroyed. Clearing the account's sync cache rescans from genesis and
recovers them. But until you do, the wallet quietly understates what you own. If
shielded receives before that point are possible, prefer `--birthday-date` with a
...
| `moth wallet import [--name <name>]` | Import from recovery phrase (stdin or interactive) or `--seed-hex`; birthday via `--birthday-discover` / `--birthday-date` / `--birthday-height` / `--birthday-tip` |
