...
5. **The one opt-in exception** is the browser extension's offscreen key-holder, which Chrome tears down and recreates at will: `WalletKeys` cannot cross the runtime-message boundary, so the background must hold a serializable secret to re-derive from, and that secret is the seed. `WalletManager.exportSeedHex(name, passphrase)` returns it against the passphrase; `WalletManager.unlockWithSeedHex(name, passphrase)` returns it beside a seed-free `UnlockedWallet` from a single keystore decrypt (the extension previously called `unlock()` then `exportSeedHex()`, paying the scrypt KDF twice per password entry). `unlock()` itself stays seed-free for every other caller.
