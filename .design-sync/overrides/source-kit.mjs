// forked from design-sync lib/source-kit.mjs — exclude components/screens/ from the synth
// entry: app screens import the wallet SDK (ledger WASM) and must not enter the DS bundle.
...
const NON_IMPL_RX = /\.(stories|test|spec)\.|\/screens\//;
...
export async function resolvePackage(ctx) {
  ...
  if (srcRoot) {
    for (const c of components) {
      ...
      // a generic container name — else 'general'. FORK: JSDoc @category takes
      // precedence — this repo's dirs (ui/, moth/) aren't its taxonomy; the
      // source JSDoc carries the real groups.
      c.group = slug(
